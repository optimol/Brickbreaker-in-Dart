...
$$.Brick = {"":
 ["color", "y", "x", "destroyInterval", "destroyed"],
 super: "Object",
 destroyTick$0: function() {
  $.ctx.clearRect$4(this.x, this.y, 50, 50);
  $.window().clearInterval$1(this.destroyInterval);
},
 get$destroyTick: function() { return new $.BoundClosure(this, 'destroyTick$0'); },
 destroy$0: function() {
  $.ctx.drawImage$3($.exploimg, this.x, this.y);
  this.destroyInterval = $.window().setInterval$2(this.get$destroyTick(), 65);
},
 collision$2: function(ballX, ballY) {
  if (typeof ballX !== 'number')
    return this.collision$2$bailout(1, ballX, ballY, 0, 0, 0, 0);
  ...
  var t1 = $.ballRad;
  if (typeof t1 !== 'number')
    return this.collision$2$bailout(2, ballX, ballY, t1, 0, 0, 0);
  t1 = ballX + t1;
  var t3 = this.x;
  if (typeof t3 !== 'number')
    return this.collision$2$bailout(3, ballX, ballY, t3, t1, 0, 0);
  if (!(t1 < t3)) {
    t1 = $.ballRad;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(4, ballX, ballY, t3, t1, 0, 0);
    t1 = ballX - t1;
    var t4 = $.Brick_brickW;
    if (typeof t4 !== 'number')
      return this.collision$2$bailout(5, ballX, ballY, t1, t4, t3, 0);
    t1 = t1 > t3 + t4;
  } else
    t1 = true;
  if (t1)
  ...
  t1 = $.ballRad;
  if (typeof t1 !== 'number')
    return this.collision$2$bailout(6, ballX, ballY, t1, t3, 0, 0);
  t1 = ballY + t1;
  t4 = this.y;
  if (typeof t4 !== 'number')
    return this.collision$2$bailout(7, ballX, t4, ballY, t3, t1, 0);
  if (!(t1 < t4)) {
    t1 = $.ballRad;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(8, ballX, t4, ballY, t1, t3, 0);
    t1 = ballY - t1;
    var t5 = $.Brick_brickH;
    if (typeof t5 !== 'number')
      return this.collision$2$bailout(9, ballX, t4, ballY, t1, t5, t3);
    t1 = t1 > t4 + t5;
  } else
    t1 = true;
  if (t1)
    return;
  this.destroyed = true;
  $.ctx.clearRect$4(t3, t4, $.Brick_brickW, $.Brick_brickH);
  t1 = $.score;
  if (typeof t1 !== 'number')
    return this.collision$2$bailout(10, ballX, t4, ballY, t3, t1, 0);
  $.score = t1 + 1;
  t1 = $.score;
  if (typeof t1 !== 'number')
    return this.collision$2$bailout(11, ballX, t4, ballY, t1, t3, 0);
  if (t1 === 25) {
    $.finalScore.get$style().set$visibility('');
    t1 = $.S($.score);
    $.query('#scorefin').set$text(t1);
    $.controls.get$style().set$visibility('hidden');
    $.window().alert$1('You Win!!!');
    $.window().clearInterval$1($.ballInterval);
    $.window().clearInterval$1($.keyInterval);
    $.window().clearInterval$1($.randBrickInterval);
    return;
  }
  t1 = $.S($.score);
  $.scoreDisplay.set$text(t1);
  t1 = $.ballRad;
  if (typeof t1 !== 'number')
    return this.collision$2$bailout(12, ballX, t4, ballY, t1, t3, 0);
  if (ballX + t1 > t3) {
    t1 = $.ballRad;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(13, ballX, t4, ballY, t1, t3, 0);
    t1 = ballX - t1;
    t5 = $.Brick_brickW;
    if (typeof t5 !== 'number')
      return this.collision$2$bailout(14, t4, ballY, t1, t3, t5, 0);
    t1 = t1 < t3 + t5;
  } else
    t1 = false;
  if (t1) {
    t1 = $.ballRad;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(15, t4, ballY, t1, 0, 0, 0);
    if (!(ballY - t1 < t4)) {
      t1 = $.ballRad;
      if (typeof t1 !== 'number')
        return this.collision$2$bailout(16, t4, ballY, t1, 0, 0, 0);
      t1 = ballY + t1;
      t3 = $.Brick_brickH;
      if (typeof t3 !== 'number')
        return this.collision$2$bailout(17, t1, t4, t3, 0, 0, 0);
      t1 = t1 > t4 + t3;
    } else
      t1 = true;
  } else
  ...
  if (t1) {
    t1 = $.ballVy;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(18, t1, 0, 0, 0, 0, 0);
    $.ballVy = t1 * -1;
  } else {
    t1 = $.ballVx;
    if (typeof t1 !== 'number')
      return this.collision$2$bailout(19, t1, 0, 0, 0, 0, 0);
    $.ballVx = t1 * -1;
  }
  this.destroy$0();
},
 collision$2$bailout: function(state, env0, env1, env2, env3, env4, env5) {
  switch (state) {
    case 1:
      var ballX = env0;
      var ballY = env1;
  ...
  switch (state) {
    ...
    case 8:
    case 9:
      if (state === 9 || state === 8 || state === 0 && !$.ltB(t1, t4))
        switch (state) {
          case 0:
            t1 = $.ballRad;
          case 8:
    ...
    case 13:
    case 14:
      if (state === 14 || state === 13 || state === 0 && $.gtB($.add(ballX, t1), t3))
        switch (state) {
          case 0:
            t1 = $.ballRad;
          case 13:
    ...
    case 16:
    case 17:
      if (state === 17 || state === 16 || state === 15 || state === 0 && t1)
        switch (state) {
          case 0:
            t1 = $.ballRad;
          case 15:
            state = 0;
          default:
            if (state === 17 || state === 16 || state === 0 && !$.ltB($.sub(ballY, t1), t4))
              switch (state) {
                case 0:
                  t1 = $.ballRad;
                case 16:
 ...
 Brick$2: function(x, y) {
  var sb = $.StringBufferImpl$('');
  sb.add$1('#');
  for (var i = 0; i < 6; ++i)
    sb.add$1($.index($.rand, $.toInt($.mul($.Math_random(), 15))));
  var t1 = sb.toString$0();
  $.ctx.set$fillStyle(t1);
  $.ctx.beginPath$0();
  $.ctx.fillRect$4(x, y, $.Brick_brickW, $.Brick_brickH);
  this.x = x;
  this.y = y;
}
};
...
$$.run_anon2 = {"":
 ...
 super: "Closure",
 call$1: function(e) {
  $.level = $.add($.Math_parseInt($.slider.get$value()), 1);
  var sign = $.ltB($.ballVx, 0) ? -1 : 1;
  var t1 = $.level;
  if (typeof t1 !== 'number')
    throw $.iae(t1);
  $.ballVx = (3 + 0.5 * t1) * sign;
  sign = $.ltB($.ballVy, 0) ? -1 : 1;
  t1 = $.level;
  if (typeof t1 !== 'number')
    throw $.iae(t1);
  $.ballVy = (5 + 0.5 * t1) * sign;
}
};
...
$$.run_anon5 = {"":
 ...
 call$1: function(e) {
  if ($.paused === true) {
    $.paused = false;
    $.ballInterval = $.window().setInterval$2($.ballTick, 30);
    $.keyInterval = $.window().setInterval$2($.keyTick, 30);
    $.randBrickInterval = $.window().setInterval$2($.randBrickTick, 1000);
  } else {
    $.paused = true;
    $.window().clearInterval$1($.ballInterval);
    $.window().clearInterval$1($.keyInterval);
...
$.ballTick = function() {
  $.ctx.beginPath$0();
  $.ctx.clearRect$4($.sub($.sub($.ballX, $.ballRad), 1), $.sub($.sub($.ballY, $.ballRad), 1), $.add($.mul($.ballRad, 2), 2), $.add($.mul($.ballRad, 2), 2));
  $.ctx.closePath$0();
  $.ctx.set$fillStyle('#000000');
  $.ctx.fillRect$4($.padX, $.padY, $.padWidth, $.padHeight);
  $.ballX = $.add($.ballX, $.ballVx);
  $.ballY = $.add($.ballY, $.ballVy);
  $.ctx.set$fillStyle('#FF0000');
  $.ctx.beginPath$0();
  $.ctx.arc$6($.ballX, $.ballY, $.ballRad, 0, 6.283185307179586, false);
  $.ctx.fill$0();
  $.ctx.closePath$0();
  if ($.leB($.ballX, $.ballRad) || $.geB($.ballX, $.sub($.width, $.ballRad)))
    $.ballVx = $.neg($.ballVx);
  if ($.leB($.ballY, $.ballRad))
    $.ballVy = $.neg($.ballVy);
  if ($.geB($.ballX, $.padX) && $.leB($.ballX, $.add($.padX, $.padWidth)) && $.geB($.ballY, $.sub($.height, 15))) {
    $.ballVy = $.neg($.ballVy);
    $.ctx.set$fillStyle('#000');
  }
  for (var t1 = $.iterator($.brickList); t1.hasNext$0() === true;)
    t1.next$0().collision$2($.ballX, $.ballY);
  if ($.gtB($.ballY, $.sub($.height, 3))) {
    t1 = $.S($.score);
    $.query('#scorefin').set$text(t1);
    $.controls.get$style().set$visibility('hidden');
    $.finalScore.get$style().set$visibility('');
    $.window().alert$1('You lose!!!');
    $.window().clearInterval$1($.ballInterval);
    $.window().clearInterval$1($.keyInterval);
    $.window().clearInterval$1($.randBrickInterval);
  }
...
$.keyTick = function() {
  if ($.keyLeftArrow === true) {
    if ($.gtB($.padX, 0)) {
      $.ctx.clearRect$4($.sub($.padX, 2), $.sub($.padY, 1), $.add($.padWidth, 3), $.add($.padHeight, 1));
      $.padX = $.sub($.padX, $.padJump);
      $.ctx.set$fillStyle('#000');
      $.ctx.fillRect$4($.padX, $.padY, $.padWidth, $.padHeight);
    }
  } else if ($.keyRightArrow === true)
    if ($.ltB($.add($.padX, $.padWidth), $.width)) {
      $.ctx.clearRect$4($.sub($.padX, 1), $.sub($.padY, 1), $.add($.padWidth, 1), $.add($.padHeight, 1));
      $.padX = $.add($.padX, $.padJump);
      $.ctx.set$fillStyle('#000');
      $.ctx.fillRect$4($.padX, $.padY, $.padWidth, $.padHeight);
    }
};
...
$.newGame = function() {
  $.finalScore.get$style().set$visibility('hidden');
  $.controls.get$style().set$visibility('');
  $.ctx.clearRect$4(0, 0, $.width, $.height);
  var t1 = $.level;
  if (typeof t1 !== 'number')
    throw $.iae(t1);
  $.ballVx = -(3 + 0.5 * t1);
  ...
  $.score = 0;
  var t3 = $.S($.score);
  $.scoreDisplay.set$text(t3);
  $.ballX = $.add($.padX, $.div($.padWidth, 2));
  $.ballY = $.sub($.padY, $.ballRad);
  $.ctx.fillRect$4($.padX, $.padY, $.padWidth, $.padHeight);
  $.keyRightArrow = false;
  $.keyLeftArrow = false;
  $.ballInterval = $.window().setInterval$2($.ballTick, 30);
...
$.generateField = function() {
  $.Brick_brickW = $.div($.width, 20);
  $.Brick_brickH = $.mul($.ballRad, 5);
  var by = $.div($.height, 10);
  if (typeof by !== 'number')
    return $.generateField$bailout(1, by, 0, 0);
  $.ctx.set$fillStyle('#000');
  for (var i = 0; i < 5; ++i) {
    var bx = $.div($.width, 15);
    if (typeof bx !== 'number')
      return $.generateField$bailout(2, bx, by, i);
    for (var j = 0; j < 5; ++j) {
      $.add$1($.brickList, $.Brick$(bx, by));
      var t1 = $.Brick_brickW;
      if (typeof t1 !== 'number')
        throw $.iae(t1);
      bx += 4 * t1;
...
$.run = function() {
  ...
  $.padX = $.div($.width, 2);
  $.padY = $.sub($.height, 5);
  $.padJump = $.div($.width, 37);
  $.rand = $.split('0123456789ABCDEF', '');
  $.exploimg = $._Elements_ImageElement(null, null, null);
  $.exploimg.set$src('images/explosion.png');
  $.scoreDisplay = $.document().query$1('#score');
  $.finalScore = $.document().query$1('#finalScore');
  $.brickList = $.ListFactory_List(null);
  $.slider = $.query('#slider');
  $.controls = $.query('#gameplay');
  $.add$1($.document().get$on().get$keyDown(), new $.run_anon());
  $.add$1($.document().get$on().get$keyUp(), new $.run_anon0());
  $.add$1($.exploimg.get$on().get$load(), new $.run_anon1());
  $.slider.get$on().get$change().add$2(new $.run_anon2(), true);
  $.add$1($.query('#reset').get$on().get$click(), new $.run_anon3());
  $.add$1($.query('#newgame').get$on().get$click(), new $.run_anon4());
  $.add$1($.query('#pause').get$on().get$click(), new $.run_anon5());
};
...
$.Brick$ = function(x, y) {
  var t1 = new $.Brick(null, null, null, null, false);
  t1.Brick$2(x, y);
  return t1;
};
...
$.randBrickTick = function() {
  $.ctx.set$fillStyle('#000');
  $.div($.height, 10);
  $.add$1($.brickList, $.Brick$($.mul($.div($.width, 10), $.toInt($.div($.mul($.Math_random(), $.width), 1.25))), $.mul($.div($.height, 10), $.toInt($.div($.mul($.Math_random(), $.height), 1.25)))));
};
...
$.generateField$bailout = function(state, env0, env1, env2) {
  switch (state) {
    case 1:
      by = env0;
      break;
...
$.keyTick.call$0 = $.keyTick;
$.keyTick.$name = "keyTick";
...
$.randBrickTick.call$0 = $.randBrickTick;
$.randBrickTick.$name = "randBrickTick";
$.ballTick.call$0 = $.ballTick;
$.ballTick.$name = "ballTick";
